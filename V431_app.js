...
function treeImagePoint(t){
  ...
  const sig=mapBoundsSig()+'|'+ps+'|'+mobileMapMode+'|'+activeMobileMapKey;let v=treeImageCache.get(t);
  if(v&&v.sig===sig)return v.p;
  const p=placementImagePoint(t)||geoToImage(t.latitude,t.longitude);treeImageCache.set(t,{sig,p});return p
}
