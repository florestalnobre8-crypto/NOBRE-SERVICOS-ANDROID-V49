...
function drawGestureTreesFast(ctx,c,pts){
  // V4.21: mantém as árvores visíveis durante giro/pinça sem redesenhar rótulos e contornos pesados.
  const dpr=c._dpr||1,zr=mapState.scale/Math.max(mapState.minScale,.0001);
  const r=Math.max(1.25*dpr,Math.min(2.5*dpr,1.15*dpr*zr));
  const groups=new Map();
  for(const t of pts){
    if(!insideBounds(t))continue;
    const p=imageToScreen(geoToImage(t.latitude,t.longitude));
    if(p.x<-6||p.x>c.width+6||p.y<-6||p.y>c.height+6)continue;
    const color=STATUS_COLOR[t.status]||STATUS_COLOR[STATUS.SEM];
    let arr=groups.get(color);if(!arr){arr=[];groups.set(color,arr)}arr.push(p);
  }
  for(const [color,arr] of groups){
    ctx.beginPath();
    for(const p of arr){ctx.moveTo(p.x+r,p.y);ctx.arc(p.x,p.y,r,0,Math.PI*2)}
    ctx.fillStyle=color;ctx.fill();
  }
}

function drawGestureGpsFast(ctx,c){
  // V4.22: mantém o cursor GPS visível durante zoom/giro sem redesenhar overlays pesados.
...
async function refreshCloud(show=true){
  ...
  async function readRemote(url,useKey){
    ...
    const lastMsFast=Number(localStorage.getItem('nobre-cloud-ms-v420')||0);
  ...
  try{
    ...
    const lastSig=localStorage.getItem('nobre-cloud-sig-v420')||'';
    const lastMs=Number(localStorage.getItem('nobre-cloud-ms-v420')||0);
    ...
    if(Array.isArray(decoded?.data)){
      ...
      localStorage.setItem('nobre-cloud-ms-v420',String(remoteMs||Date.now()));
      localStorage.setItem('nobre-cloud-sig-v420',sig);
      localStorage.setItem('nobre-cloud-last-ok-v420',new Date().toISOString());

      if(show)showToast(`Dados atualizados: ${allTrees.length.toLocaleString('pt-BR')} árvores.`);
    }
  }catch(e){
    console.warn('Aplicação da sincronização V4.22:',e);
...
boot().catch(err=>{console.error('Falha na inicialização V4.22:',err);try{goScreen('home')}catch(_){};setTimeout(()=>{try{fastRefreshV45(false)}catch(_){}},1000)});
