...
async function refreshCloud(show=true){
  ...
  async function readRemote(url,useKey){
    ...
    return {url,key:useKey,r,raw,decoded};
  }
  ...
  try{
    ...
    const lastSig=localStorage.getItem('nobre-cloud-sig-v419')||'';
    const lastMs=Number(localStorage.getItem('nobre-cloud-ms-v419')||0);
    ...
    if(Array.isArray(decoded?.data)){
      ...
      localStorage.setItem('nobre-cloud-ms-v419',String(remoteMs||Date.now()));
      localStorage.setItem('nobre-cloud-sig-v419',sig);
      localStorage.setItem('nobre-cloud-last-ok-v419',new Date().toISOString());

      if(show)showToast(`Dados atualizados: ${allTrees.length.toLocaleString('pt-BR')} árvores.`);
    }
  }catch(e){
    console.warn('Aplicação da sincronização V4.19:',e);
...
window.addEventListener('error',e=>{
  try{
    console.error('Erro V4.19:',e.error||e.message);
    showToast('O app encontrou um erro. Feche e abra novamente.');
  }catch(_){}
});
window.addEventListener('unhandledrejection',e=>{
  console.warn('Promessa V4.19:',e.reason);
});
