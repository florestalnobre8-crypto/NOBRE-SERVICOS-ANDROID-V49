...
async function refreshCloud(show=true){
  ...
  try{
    ...
    if(Array.isArray(decoded?.data)){
      // V4.28 FINAL: atualiza dados/mapas recebidos do PC sem sobrescrever o mapa que o usuário deixou em uso no celular.
    ...
    console.warn('Aplicação da sincronização V4.28:',e);
...
boot().catch(err=>{console.error('Falha na inicialização V4.28:',err);try{goScreen('home')}catch(_){};setTimeout(()=>{try{fastRefreshV45(false)}catch(_){}},1000)});
