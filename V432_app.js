...
let mobileMaps={},activeMobileMapKey='',mobileMapMode='single',mobileCompositePackage=null,mobileCompositeSignature='',syncedPcAllMapsPackage=null,pcTreePlacement=null;
...
function placementTreeKey(t){return String(t?.key||((t?.ut??'')+'|'+(t?.arvore??''))).trim()}
function placementApplies(){
  const p=pcTreePlacement;if(!p||!p.positions||typeof p.positions!=='object')return false;
  if(String(p.mode||'single')!==String(mobileMapMode||'single'))return false;
  if(mobileMapMode==='all')return true;
  const pk=mobileMapKey({key:p.mapKey||''}),ak=mobileMapKey({key:activeMobileMapKey||''});
  if(pk&&ak&&pk===ak)return true;
  const pn=norm(String(p.mapName||p.mapLabel||'')),mn=norm(String(mapPackage?.name||mobileMapLabel(activeMobileMapKey,mobileMaps[activeMobileMapKey])||''));
  return !!(pn&&mn&&(pn===mn||pn.includes(mn)||mn.includes(pn)));
}
function placementImagePoint(t){
  if(!placementApplies())return null;
  const a=pcTreePlacement.positions?.[placementTreeKey(t)];
  if(!Array.isArray(a)||a.length<2)return null;
  const u=Number(a[0]),v=Number(a[1]);if(!Number.isFinite(u)||!Number.isFinite(v))return null;
  const w=mapState.imgReady?(mapState.img.naturalWidth||mapState.img.width):1600,h=mapState.imgReady?(mapState.img.naturalHeight||mapState.img.height):1200;
  return{x:u*w,y:v*h};
}
...
async function applyMobileMapsFromPayload(payload,preserveUserChoice=true){
  // V4.31: recebe a posição exata das árvores calculada no PC.
  if(payload?.pcTreePlacement?.positions&&typeof payload.pcTreePlacement.positions==='object'){
    pcTreePlacement={...payload.pcTreePlacement,bounds:normalizeBounds(payload.pcTreePlacement.bounds)||null};
  }
  ...
  incoming.forEach((m,i)=>{
    ...
    next[key]={...m,key,bounds:{...normalizeBounds(m.bounds)}};
  });

  // V4.30 — ENCAIXE IDÊNTICO AO PC
  // O PC envia em payload.map exatamente o mapa que está aberto na tela,
  // com os limites (mapBounds) realmente usados para desenhar as árvores.
  // payload.maps pode conter uma cópia persistida mais antiga desse mesmo mapa.
  // No modo de um mapa, a cópia exata da tela do PC TEM prioridade.
  if(payload?.selectedMapMode!=='all' && payload?.map){
    const exact=payload.map, exactBounds=normalizeBounds(exact.bounds);
    if(exactBounds && (exact.dataUrl||exact.src)){
      const rawSelected=String(payload?.selectedMapKey||'').trim();
      const normalizedSelected=rawSelected?mobileMapKey({key:rawSelected}):'';
      const labelWanted=norm(String(payload?.selectedMapLabel||exact.fileLabel||exact.name||''));
      let exactKey='';
      if(rawSelected && next[rawSelected])exactKey=rawSelected;
      else if(normalizedSelected && next[normalizedSelected])exactKey=normalizedSelected;
      else{
        exactKey=Object.keys(next).find(k=>{
          const m=next[k];
          return (normalizedSelected&&mobileMapKey({key:k})===normalizedSelected) ||
                 (labelWanted&&norm(String(mobileMapLabel(k,m)))===labelWanted);
        })||'';
      }
      if(!exactKey){
        exactKey=normalizedSelected||mobileMapKey(exact,0);
      }
      const previous=next[exactKey]||{};
      next[exactKey]={...previous,...exact,key:exactKey,bounds:{...exactBounds},pcExactView:true};
    }
  }
  ...
  const hasPcPlacement=!!(pcTreePlacement?.positions&&Object.keys(pcTreePlacement.positions).length);
  const preferredMode=hasPcPlacement?(payload?.selectedMapMode||pcTreePlacement?.mode||'single'):(pref?.mode||payload?.selectedMapMode||'single');
  const payloadRawKey=String(payload?.selectedMapKey||'').trim();
  const payloadNormKey=payloadRawKey?mobileMapKey({key:payloadRawKey}):'';
  const prefRawKey=String(pref?.key||'').trim();
  const prefNormKey=prefRawKey?mobileMapKey({key:prefRawKey}):'';
  const payloadLabel=norm(String(payload?.selectedMapLabel||''));
  const findByLabel=()=>Object.keys(mobileMaps).find(k=>payloadLabel&&norm(String(mobileMapLabel(k,mobileMaps[k])))===payloadLabel)||'';
  const preferredKey=hasPcPlacement
    ?((payloadRawKey&&mobileMaps[payloadRawKey])?payloadRawKey
      :((payloadNormKey&&mobileMaps[payloadNormKey])?payloadNormKey:(findByLabel()||entries[0][0])))
    :((prefRawKey&&mobileMaps[prefRawKey])?prefRawKey
      :((prefNormKey&&mobileMaps[prefNormKey])?prefNormKey
      :((payloadRawKey&&mobileMaps[payloadRawKey])?payloadRawKey
      :((payloadNormKey&&mobileMaps[payloadNormKey])?payloadNormKey
      :(findByLabel()||entries[0][0])))));

  activeMobileMapKey=preferredKey;
  if(hasPcPlacement&&pcTreePlacement?.bounds){
    const pb=normalizeBounds(pcTreePlacement.bounds);
    if(preferredMode==='all'&&syncedPcAllMapsPackage&&pb)syncedPcAllMapsPackage={...syncedPcAllMapsPackage,bounds:{...pb}};
    if(preferredMode!=='all'&&mobileMaps[preferredKey]&&pb)mobileMaps[preferredKey]={...mobileMaps[preferredKey],bounds:{...pb}};
  }
...
function visibleMapTrees(){
  ...
  const placementKeys=placementApplies()?pcTreePlacement?.positions:null;
  const items=placementKeys
    ?planTreesCache.filter(t=>Object.prototype.hasOwnProperty.call(placementKeys,placementTreeKey(t)))
    :((mobileMapMode==='all'&&entries.length)
...
function treeImagePoint(t){
  // V4.32: nunca guarda posição calculada antes da imagem real do mapa terminar de carregar.
  // Antes, na primeira abertura, o app usava o tamanho provisório 1600x1200 e mantinha
  // essas coordenadas no cache. Ao tocar em "Usar mapa", o cache era recriado com o
  // tamanho real da imagem e tudo se alinhava. Agora a primeira abertura já faz isso certo.
  if(!mapState.imgReady){
    return placementImagePoint(t)||geoToImage(t.latitude,t.longitude);
  }
  const ps=pcTreePlacement?.updatedAt||pcTreePlacement?.version||'';
  const iw=mapState.img.naturalWidth||mapState.img.width||0;
  const ih=mapState.img.naturalHeight||mapState.img.height||0;
  const sig=mapBoundsSig()+'|'+ps+'|'+mobileMapMode+'|'+activeMobileMapKey+'|'+iw+'x'+ih;
  let v=treeImageCache.get(t);
  if(v&&v.sig===sig)return v.p;
  const p=placementImagePoint(t)||geoToImage(t.latitude,t.longitude);
  treeImageCache.set(t,{sig,p});
  return p
}
...
function loadMapImage(pkg=mapPackage,fit=true){if(!pkg)return;const b=normalizeBounds(pkg.bounds);if(b){mapBounds={...b};invalidateMapGeometry();updateMapBoundsInputs()}if(fit)mapState.fitOnNextVisible=true;mapState.imgReady=false;mapState.img.onload=()=>{mapState.imgReady=true;invalidateMapGeometry();const r=mapState.canvas?.getBoundingClientRect?.();const visible=!!(r&&r.width>20&&r.height>20&&$('mapScreen')?.classList.contains('active'));if(mapState.fitOnNextVisible&&visible)resizeMap(true);else drawMapSoon()};mapState.img.onerror=()=>{mapState.imgReady=false;invalidateMapGeometry();drawMapSoon()};const src=pkg.dataUrl||pkg.src;if(src)mapState.img.src=src;else drawMapSoon();updateProjectUi()}
function initGeoMap(){
...
function drawGestureGpsFast(ctx,c){
  ...
  if(navTarget&&validCoord(navTarget.latitude,navTarget.longitude)){
    const tp=imageToScreen(treeImagePoint(navTarget));
    ctx.save();
...
function focusOnTree(t,zoom=6){if(!validCoord(t.latitude,t.longitude)){showToast('Coordenada inválida.');return}const p=treeImagePoint(t),target=Math.min(mapState.maxScale,Math.max(mapState.minScale,mapState.minScale*zoom)),vc=mapVisibleCenter();mapState.scale=target;const q=screenToBase(vc.x,vc.y);mapState.offsetX=q.x-p.x*target;mapState.offsetY=q.y-p.y*target;drawMapSoon()}
function fitNavigationView(force=false){if(!navTarget||!currentGps||!mapState.canvas)return;const now=Date.now();if(!force&&now-lastNavFitAt<1200)return;lastNavFitAt=now;const c=mapState.canvas,dpr=c._dpr||1,v=mapVisibleViewport(),vc=mapVisibleCenter(),a=geoToImage(currentGps.lat,currentGps.lon),b=treeImagePoint(navTarget),dx=Math.max(Math.abs(a.x-b.x),45),dy=Math.max(Math.abs(a.y-b.y),45),padX=60*dpr,padY=55*dpr;let scale=Math.min((v.width-padX*2)/dx,(v.height-padY*2)/dy,mapState.maxScale);if(haversine(currentGps,{lat:navTarget.latitude,lon:navTarget.longitude})<80)scale=Math.min(mapState.maxScale,Math.max(scale,mapState.minScale*9));scale=Math.max(mapState.minScale*.08,scale);mapState.scale=scale;const midX=(a.x+b.x)/2,midY=(a.y+b.y)/2,q=screenToBase(vc.x,vc.y);mapState.offsetX=q.x-midX*scale;mapState.offsetY=q.y-midY*scale;drawMapSoon()}
...
function buildPayload(){
  return{
    ...
    pcTreePlacement:pcTreePlacement?{...pcTreePlacement}:null,
    data:allTrees
  }
}
...
async function refreshCloud(show=true){
  ...
  try{
    ...
    if(Array.isArray(decoded?.data)){
      // V4.29 FINAL: atualiza dados/mapas recebidos do PC sem sobrescrever o mapa que o usuário deixou em uso no celular.
    ...
    console.warn('Aplicação da sincronização V4.29:',e);
...
boot().catch(err=>{console.error('Falha na inicialização V4.29:',err);try{goScreen('home')}catch(_){};setTimeout(()=>{try{fastRefreshV45(false)}catch(_){}},1000)});
