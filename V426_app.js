...
async function activateAllMobileMaps(fit=true,savePref=true,notify=true){
  ...
  // V4.26 FINAL: quando o PC enviou o modo TODOS OS MAPAS, usa exatamente
...
async function applyMobileMapsFromPayload(payload,preserveUserChoice=true){
  // V4.26: guarda o mosaico exato que veio do PC quando ele está em TODOS OS MAPAS.
...
async function refreshCloud(show=true){
  ...
  try{
    ...
    console.warn('Aplicação da sincronização V4.26:',e);
...
async function boot(){
  ...
  if(local?.schemaVersion>=2&&local?.data?.length){
    await applyMobileMapsFromPayload(local,false);
...
boot().catch(err=>{console.error('Falha na inicialização V4.26:',err);try{goScreen('home')}catch(_){};setTimeout(()=>{try{fastRefreshV45(false)}catch(_){}},1000)});
