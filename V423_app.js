...
window.__onNativeHeading=function(deg){
  ...
  if(!Number.isFinite(currentDeviceHeading))currentDeviceHeading=target;
    ...
    currentDeviceHeading=(currentDeviceHeading+diff*.58+360)%360;
  }
...
function visibleMapTrees(){
  ...
  const sig=`${treeDataRevision}|${mapBoundsSig()}`;
  if(visibleTreeCache.sig===sig)return visibleTreeCache.items;
  const items=planTreesCache.filter(insideBounds);visibleTreeCache={sig,items};return items
}
...
const mapState={canvas:null,ctx:null,img:new Image(),imgReady:false,scale:1,offsetX:0,offsetY:0,rotation:0,minScale:.1,maxScale:16,dragging:false,lastX:0,lastY:0,moved:false,raf:0,gestureActive:false};
...
function loadMapImage(pkg=mapPackage,fit=true){if(!pkg)return;const b=normalizeBounds(pkg.bounds);if(b){mapBounds={...b};invalidateMapGeometry();updateMapBoundsInputs()}mapState.imgReady=false;mapState.img.onload=()=>{mapState.imgReady=true;if(fit)resizeMap(true);else drawMapSoon()};mapState.img.onerror=()=>{mapState.imgReady=false;drawMapSoon()};const src=pkg.dataUrl||pkg.src;if(src)mapState.img.src=src;else drawMapSoon();updateProjectUi()}
function initGeoMap(){
...
function resizeMap(fit=false){const c=mapState.canvas;if(!c)return;const r=c.getBoundingClientRect(),maxDpr=isAndroidApp()?1.35:2,dpr=Math.min(devicePixelRatio||1,maxDpr);c.width=Math.max(1,Math.round(r.width*dpr));c.height=Math.max(1,Math.round(r.height*dpr));c._dpr=dpr;if(navTarget&&currentGps&&navigationFollow)fitNavigationView(true);else if(fit)fitGeoMap();else drawMapSoon()}
...
function mapVisibleCenter(){const v=mapVisibleViewport();return{x:v.width/2,y:v.height/2}}
function fitGeoMap(){const c=mapState.canvas;if(!c)return;if(!mapState.imgReady){const b=treeBounds(.04);if(b){mapBounds=b;mapPackage.bounds={...b};invalidateMapGeometry()}drawMapSoon();return}const v=mapVisibleViewport(),s=Math.min(v.width/mapState.img.width,v.height/mapState.img.height);mapState.scale=s;mapState.minScale=s*.55;mapState.maxScale=s*40;const vc=mapVisibleCenter(),q=screenToBase(vc.x,vc.y);mapState.offsetX=q.x-mapState.img.width*s/2;mapState.offsetY=q.y-mapState.img.height*s/2;drawMapSoon()}
...
function prepareGestureTreePaths(){
  if(typeof Path2D==='undefined'||!mapTreesVisible)return null;
  const c=mapState.canvas;if(!c)return null;
  const sig=`${treeDataRevision}|${mapBoundsSig()}|${(c._dpr||1).toFixed(2)}`;
  if(gestureTreePathCache.sig===sig&&gestureTreePathCache.paths)return gestureTreePathCache.paths;
  const dpr=c._dpr||1,r=Math.max(.6,.78*dpr/Math.max(mapState.minScale,.000001));
  const paths={};for(const color of Object.values(STATUS_COLOR))paths[color]=new Path2D();
  for(const t of visibleMapTrees()){
    const p=treeImagePoint(t),color=STATUS_COLOR[t.status]||STATUS_COLOR[STATUS.SEM],path=paths[color]||(paths[color]=new Path2D());
    path.moveTo(p.x+r,p.y);path.arc(p.x,p.y,r,0,Math.PI*2)
  }
  gestureTreePathCache={sig,paths};return paths
}
function drawGestureTreesFast(ctx,c,pts){
  // V4.23: camada vetorial cacheada — mantém todas as cores visíveis e reduz travadas no giro/pinça/arraste.
  const paths=prepareGestureTreePaths();
  if(paths){
    const cc=mapCanvasCenter();ctx.save();ctx.translate(cc.x,cc.y);ctx.rotate(mapState.rotation||0);ctx.translate(-cc.x,-cc.y);ctx.translate(mapState.offsetX,mapState.offsetY);ctx.scale(mapState.scale,mapState.scale);
    for(const [color,path] of Object.entries(paths)){ctx.fillStyle=color;ctx.fill(path)}ctx.restore();return
  }
  const dpr=c._dpr||1,r=1.7*dpr,groups=new Map();
  for(const t of visibleMapTrees()){const p=imageToScreen(treeImagePoint(t));if(p.x<-6||p.x>c.width+6||p.y<-6||p.y>c.height+6)continue;const color=STATUS_COLOR[t.status]||STATUS_COLOR[STATUS.SEM];let arr=groups.get(color);if(!arr){arr=[];groups.set(color,arr)}arr.push(p)}
  for(const [color,arr] of groups){ctx.beginPath();for(const p of arr){ctx.moveTo(p.x+r,p.y);ctx.arc(p.x,p.y,r,0,Math.PI*2)}ctx.fillStyle=color;ctx.fill()}
}

function drawGestureGpsFast(ctx,c){
  // V4.23: mantém o cursor GPS visível durante zoom/giro sem redesenhar overlays pesados.
...
async function refreshCloud(show=true){
  ...
  async function readRemote(url,useKey){
    ...
    const lastMsFast=Number(localStorage.getItem('nobre-cloud-ms-v423')||0);
  ...
  try{
    ...
    const lastSig=localStorage.getItem('nobre-cloud-sig-v423')||'';
    const lastMs=Number(localStorage.getItem('nobre-cloud-ms-v423')||0);
    ...
    if(Array.isArray(decoded?.data)){
      ...
      localStorage.setItem('nobre-cloud-ms-v423',String(remoteMs||Date.now()));
      localStorage.setItem('nobre-cloud-sig-v423',sig);
      localStorage.setItem('nobre-cloud-last-ok-v423',new Date().toISOString());

      if(show)showToast(`Dados atualizados: ${allTrees.length.toLocaleString('pt-BR')} árvores.`);
    }
  }catch(e){
    console.warn('Aplicação da sincronização V4.23:',e);
...
boot().catch(err=>{console.error('Falha na inicialização V4.23:',err);try{goScreen('home')}catch(_){};setTimeout(()=>{try{fastRefreshV45(false)}catch(_){}},1000)});
